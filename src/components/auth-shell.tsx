...
export function AuthShell({ title, subtitle, children, footer }: { title: string; subtitle: string; children: ReactNode; footer: ReactNode }) {
  return <main className="auth-page">
    ...
    <section className="form-panel"><div className="auth-tools"><LanguageSwitcher /></div><div className="auth-card"><div className="mobile-brand"><ShieldCheck/> مدار CRM</div><h2>{title}</h2><p className="muted">{subtitle}</p>{children}<div className="auth-footer">{footer}</div></div></section>
  </main>;
}
